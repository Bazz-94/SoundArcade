...
public sealed class RiverRunSession
{
  ...
  public IReadOnlyList<RunEvent> Update(float deltaTimeSeconds)
  {
    ...
    RunObstacle? spawned = this.spawner.Update(deltaTimeSeconds, this.elapsedSeconds);

    if (spawned.HasValue)
    {
      this.obstacles.Add(spawned.Value);
      events.Add(new PlaySoundEvent(
        RunConstants.SoundId.ObstacleSpawn,
        LaneToPosition(spawned.Value.Lane, spawned.Value.Z),
