...
task:
  ...
  description: Add RiverRun-specific audio behavior that uses left and right river ambience for lane orientation and maps core gameplay and input events to simple generated sounds that remain readable alongside speech.

  scope:
    includes:
      - Drive continuous left and right river ambience during RiverRun so players can infer lane position from the stereo field.
  ...
  acceptance_criteria:
    - During a run, the player can hear stable river ambience on both sides and use it to distinguish left, center, and right lane positioning.
